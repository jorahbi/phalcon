...
class SecurityPlugin extends Plugin
{
    ...
    protected function getRole()
    {
        ...
        if (!is_dir($config->application->cacheDir . $config->cache->cacheDir)) {
            @mkdir($config->application->cacheDir . $config->cache->cacheDir , 0755, true);
        }
    	$cache->save($config->cache->cacheDir . 'admin.role.cache', $roles);
    	return $roles;
    }
}